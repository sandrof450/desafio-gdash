...
import type { ApiRegisterRequest, ApiLoginRequest } from 'src/constants/auth/types';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const insertRegister = async (data: ApiRegisterRequest) => {
    ...
    try {
      ...
      return {success: true, message: successMessage}
        ...
        const detailedMessage = (error as any).response?.data?.message || (error as Error).message || "Erro de conexão desconhecido.";
        
        //Define a mensagem padrão, amigável para o usuário.
        let userFacingMessage = "Falha na operação. Por favor, verifique os dados e tente novamente.";
        ...
        if (detailedMessage.includes('E-mail já verificado') || detailedMessage.includes('e-mail já existe')) {
  ...
  const login = async (data: ApiLoginRequest) => {
    ...
    try {
      ...
      if (!tokenData || !('accessToken' in tokenData) || !('refreshToken' in tokenData)) {
        console.error("Login falhou ou tokens não foram retornados.", tokenData);
      ...
      errorMessage = (error as any).response?.data?.message || (error as Error).message || 'Credenciais inválidas ou falha de conexão.';
  ...
  return (
    <AuthContext.Provider value={{ isAuthenticated: !!accessToken, accessToken, loading, inputRegister, login, insertRegister, logout }}>
      {children}
