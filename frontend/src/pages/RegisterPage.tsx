...
import { Mail, UserIcon, Lock, EyeOff, Eye, Check, ArrowRight, Activity } from "lucide-react";

import { registerSchema } from "../constants/auth/schemas";
import type { RegisterData } from "../constants/auth/types";
...
export function RegisterPage() {
  ...
  const { insertRegister, loading } = useAuth()
  ...
  const { strength } = usePasswordRequirements(password);
  
  const onSubmit = async ({name, lastName, email, password}: RegisterData) => {
    try {
      name = `${name} ${lastName}`;
      if (password !== confirmPassword) {
        toast.error('Senha invalida', {
          description: 'A senha de confirmação não é igual à senha.',
        });
        return;   
      }
      
      
      const result = await insertRegister({ userName: name, userEmail: email, userSenha: password });
      if (result.success) {
        ...
        toast.error('Falha no cadastro', {
          ...
          // Falha
          const errorMessage = (error as Error).message || "Credenciais inválidas.";
